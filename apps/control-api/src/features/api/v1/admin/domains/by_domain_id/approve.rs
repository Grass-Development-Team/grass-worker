...
pub(crate) fn router() -> axum::Router<crate::state::ControlApiState> {
    axum::Router::new().route("/domains/{domain_id}/approve", axum::routing::post(approve))
}
...
async fn decide(
    ...
    route_invalidation::invalidate_project(db, &secret_key, updated.project_id)
        .await
        .map_err(|source| AppError::Infrastructure { op: OP, source })?;
    Ok(ok_response(DecideResponse {
        domain: domain_view(&updated),
        reason,
    }))
}
...
fn domain_view(binding: &project_host_binding::Model) -> DomainResponse {
    DomainResponse {
        id: binding.id,
        project_id: binding.project_id,
        host: binding.host.clone(),
        status: binding_status(&binding.status),
        review_status: review_status(&binding.review_status),
