...
pub(crate) fn router() -> axum::Router<crate::state::ControlApiState> {
    axum::Router::new().route("/users/batch", axum::routing::post(batch))
}
...
pub async fn batch(
    ...
    let (ids, status) = match body {
        UserBatchRequest::Enable { ids } => (ids, UserStatus::Active),
        UserBatchRequest::Disable { ids } => (ids, UserStatus::Disabled),
    };
    let ids = normalize_ids(ids, OP)?;
    let results = run(ids, |user_id| {
        let state = state.clone();
        let session = session.clone();
        let status = status.clone();
        async move {
            crate::domain::admin_users::update(
                &state,
