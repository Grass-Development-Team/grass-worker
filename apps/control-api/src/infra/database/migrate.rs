...
mod tests {
    ...
    async fn postgres_region_catalog_backfills_and_enforces_references() -> anyhow::Result<()> {
        ...
        let result: anyhow::Result<()> = async {
            ...
            let columns = query_column_shapes(&database.db, "SELECT column_name, udt_name, is_nullable, column_default FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'regions'").await?;
    ...
    async fn postgres_domain_onboarding_migrates_and_checks_customer_dns() -> anyhow::Result<()> {
        ...
        let result: anyhow::Result<()> = async {
            ...
            db.execute_unprepared(&format!(r#"
                INSERT INTO users (id, email, display_name) VALUES ('{owner}', 'owner@example.org', 'Owner'), ('{actor}', 'adder@example.org', 'Adding user');
                INSERT INTO teams (id, slug, name, owner_user_id) VALUES ('{}', 'onboarding', 'Onboarding', '{owner}');
                INSERT INTO projects (id, team_id, slug, name, created_by_user_id) VALUES ('{}', '{}', 'onboarding', 'Onboarding', '{owner}');
                INSERT INTO regions (code, name) VALUES ('eu', 'Europe');
                INSERT INTO regional_ingresses (id, region, hostname, created_at, updated_at) VALUES ('{entry_id}', 'eu', 'entry.example.org', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
            "#, old.team_id, old.project_id, old.team_id)).await?;
            binding::ActiveModel::from(old.clone()).insert(db).await?;
            db.execute_unprepared(&format!(r#"
                INSERT INTO managed_certificates (id, ingress_id, hostname, issuer, generation) VALUES ('{entry_id}', '{entry_id}', 'entry.example.org', 'letsencrypt', '{entry_id}');
            ...
            let entry_columns = query_column_shapes(db, "SELECT column_name, udt_name, is_nullable, column_default FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'regional_ingresses'").await?;
            ensure!(!entry_columns.iter().any(|c| c.name.starts_with("certificate_") || c.name.starts_with("dns_challenge") || c.name == "tls_enabled" || c.name == "acme_account"));
            ensure!(entry_columns.iter().any(|c| c.name == "dns_checked_at" && c.udt_name == "timestamptz" && c.nullable == "YES" && c.default.is_none()));
            let columns = query_column_shapes(db, "SELECT column_name, udt_name, is_nullable, column_default FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'domain_onboarding'").await?;
            for name in ["checked_at", "lease_until"] {
                ensure!(columns.iter().any(|c| c.name == name && c.udt_name == "timestamptz" && c.nullable == "YES" && c.default.is_none()));
            }
            ensure!(columns.iter().any(|c| c.name == "next_check_at" && c.nullable == "NO" && c.default.is_some()));
            ensure!(columns.iter().any(|c| c.name == "contact_email" && c.udt_name == "text" && c.nullable == "NO"));
            ensure!(object_count(db, "SELECT count(*) AS count FROM pg_constraint WHERE conrelid = 'domain_onboarding'::regclass AND contype = 'f'").await? == 2);
            ensure!(object_count(db, "SELECT count(*) AS count FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'ix_domain_onboarding_due'").await? == 1);
            ensure!(db.execute_unprepared("UPDATE managed_certificates SET host_binding_id = NULL").await.is_err());
            ensure!(db.execute_unprepared("UPDATE managed_certificates SET challenge_method = 'dns01'").await.is_err());
            ensure!(db.execute_unprepared("UPDATE domain_onboarding SET dns_status = 'invalid'").await.is_err());
            ...
            binding::ActiveModel::from(custom.clone()).insert(db).await?;
            ...
            for (address, txt, expected, active) in [
                ...
                let bound = binding::Entity::find_by_id(custom.id).one(db).await?.unwrap();
                ensure!(check.dns_status == expected, "unexpected DNS state: {}", check.dns_status);
            ...
            let cert = managed_certificate::Entity::find_by_id(custom.id).one(db).await?.unwrap();
            ensure!(cert.hostname == custom.host && cert.contact_email == "adder@example.org" && cert.auto_renew && cert.challenge_method == "http01");
            ensure!(cert.status == "pending", "no eligible entry nodes means no external order");
            ensure!(managed_certificate::Entity::find_by_id(entry_id).one(db).await?.is_none());
            ...
            db.execute_unprepared(&format!("UPDATE domain_onboarding SET lease_until = CURRENT_TIMESTAMP + INTERVAL '1 minute' WHERE binding_id = '{}'", custom.id)).await?;
            domain_onboarding::run_check_with_resolver(db, custom.id, "secret", true, &resolver).await?;
            ensure!(domain_onboarding::get(db, custom.id).await?.unwrap().dns_status == "ready");
            // Simulate a restart after an abandoned lease. The next scheduled check recovers.
            db.execute_unprepared(&format!("UPDATE domain_onboarding SET lease_until = CURRENT_TIMESTAMP - INTERVAL '1 second', next_check_at = CURRENT_TIMESTAMP - INTERVAL '1 second' WHERE binding_id = '{}'", custom.id)).await?;
            domain_onboarding::run_check_with_resolver(db, custom.id, "secret", false, &resolver).await?;
            ensure!(domain_onboarding::get(db, custom.id).await?.unwrap().dns_status == "entry_unavailable");
            server.abort();
            let settings = certificate_settings::CertificateSettings { issuer: "zerossl".into(), eab: json!({"eab_kid":"test-id","eab_hmac_key":"c2VjcmV0"}) };
