...
- **M1.7 幂等 Seed**：初始化默认团队分组、默认个人团队、配额计划、角色、Host Policy 和审核策略；
...
- `POST /api/v1/setup/admin`：注册初始管理员，并将管理员设置为 seed 默认个人团队的默认用户；
...
Seed 不创建默认管理员。初始管理员必须由 setup flow 的 `POST /api/v1/setup/admin` 注册；注册完成后，该管理员默认归属 seed 初始化的默认个人团队。
